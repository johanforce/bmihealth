...
class ShapeRipple : View {
     ...
     */
    private var shapeRippleEntries: Deque<ShapeRippleEntry>? = null
     ...
     */
    private var rippleRandomColors: MutableList<Int>? = null
     ...
     */
    private var rippleInterpolator: Interpolator? = null

    /**
     * The random generator object for both color ([.enableRandomColor] is set to true) and position ([.enableRandomPosition] is set to true)
     */
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        rippleRandomColors = ShapePulseUtil.generateRandomColours()
        ...
        if (attrs != null) {
            ...
            try {
                ...
                enableRandomPosition =
                    ta.getBoolean(R.styleable.ConnectingRipple_enable_random_position, false)
                ...
                setEnableRandomColor(
                    ta.getBoolean(
                        R.styleable.ConnectingRipple_enable_random_color,
                        false
                    )
                )
    ...
    private fun initializeEntries(shapeRipple: BaseShape?) {
        ...
        maxRippleRadius =
            if (rippleMaximumRadius != NO_VALUE.toFloat()) rippleMaximumRadius.toInt() else min(
                viewWidth,
                viewHeight
        ...
        for (i in 0 until rippleCount) {
            val shapeRippleEntry = shapeRipple?.let { ShapeRippleEntry(it) }
            shapeRippleEntry?.x =
                (if (enableRandomPosition) random!!.nextInt(viewWidth) else viewWidth / 2)
            shapeRippleEntry?.y =
                (if (enableRandomPosition) random!!.nextInt(viewHeight) else viewHeight / 2)
            ...
            if (enableRandomColor) {
                shapeRippleEntry?.setOriginalColorValue(
                    rippleRandomColors!![random!!.nextInt(
                        rippleRandomColors!!.size
                    )]
                )
            } else {
    ...
    private fun reconfigureEntries() {
        ...
        shapePaint?.strokeWidth = rippleStrokeWidth.toFloat()
        for (shapeRippleEntry in shapeRippleEntries!!) {
            if (enableRandomColor && rippleRandomColors.isNullOrEmpty() && random != null) {
                shapeRippleEntry.setOriginalColorValue(
                    rippleRandomColors!![random!!.nextInt(
                        rippleRandomColors!!.size
                    )]
    ...
    fun start(millis: Int) {
        ...
        rippleValueAnimator?.addUpdateListener { animation ->
            render(
    ...
    private fun render(multiplierValue: Float) {
        ...
        if (shapeRippleEntries.isNullOrEmpty()) {
            return
        }
        var firstEntry: ShapeRippleEntry = shapeRippleEntries?.peekFirst()!!

        // Calculate the multiplier value of the first entry
        var firstEntryMultiplierValue: Float =
            firstEntry.multiplierValue + max(multiplierValue - lastMultiplierValue, 0f)
        ...
        if (firstEntryMultiplierValue >= 1.0f) {
            ...
            val removedEntry: ShapeRippleEntry = shapeRippleEntries!!.pop()
            removedEntry.reset()
            removedEntry.setOriginalColorValue(
                if (enableRandomColor) rippleRandomColors!![random!!.nextInt(
                    rippleRandomColors!!.size
                )] else rippleColor
            )
            ...
            firstEntryMultiplierValue = firstEntry.multiplierValue + max(
                multiplierValue - lastMultiplierValue,
                0f
            )
            firstEntry.x =
                (if (enableRandomPosition) random!!.nextInt(viewWidth) else viewWidth / 2)
    ...
    fun setRippleRandomColors(rippleRandomColors: MutableList<Int>?) {
        ...
        require(rippleRandomColors.size != 0) { "List of color cannot be empty" }
