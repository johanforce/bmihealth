...
package com.jarvis.bmihealth.presentation.register.model
...
import com.jarvis.bmihealth.presentation.common.Constant.MAX_HEIGHT_CM
import com.jarvis.bmihealth.presentation.common.Constant.MAX_STRIDE_LENGTH_CM
import com.jarvis.bmihealth.presentation.common.Constant.MAX_WEIGHT_KG
import com.jarvis.bmihealth.presentation.common.Constant.MIN_HEIGHT_CM
...
class ProfilePickerDialogModel(private val context: Context) {
    val heightPickerModelUnitOne: NumberPickerModel
        get() {
            ...
            numberPickerModelOne.unit = TypeUnits.METRIC.index
    ...
    val heightPickerModelUnitTwo: NumberPickerModel
        get() {
            ...
            numberPickerModelTwo.unit = TypeUnits.IMPERIAL.index
    ...
    fun getCurrentUserHeight(height: Double, currentUnit: Int): Pair<Int, Int> {
        val currentValue: Pair<Int, Int> = if (currentUnit == TypeUnits.METRIC.index) {
            Pair(height.toInt(), 0)
        } else {
    ...
    fun getCurrentUserStrideLength(strideLength: Double, currentUnit: Int): Pair<Int, Int> {
        return if (currentUnit == TypeUnits.METRIC.index) {
    ...
    fun convertPairValueToDouble(pairValue: Pair<Int, Int>, typeUnit: Int, typeView: Int): Double {
        ...
        if (typeUnit == TypeUnits.IMPERIAL.index && (typeView == TypeView.HEIGHT.index || typeView == TypeView.STRIDE_LENGTH.index)) {
            valueFinal =
                UnitConverter.convertFeetToInch(pairValue.first.toDouble()) + pairValue.second
        }
        if (typeView == TypeView.WEIGHT.index) {
