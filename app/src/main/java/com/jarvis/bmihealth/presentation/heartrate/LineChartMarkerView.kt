...
import com.jarvis.bmihealth.presentation.utilx.roundCelling
...
class LineChartMarkerView(
    ...
    private fun showTooltips(e: Entry) {
        if (e.y == 0f) return
        when (typeTime) {
            MeasurementGraphType.Week.index -> {
                if (e.x == 7f || e.x == 6f) {
                    showTooltipsRight()
                } else {
                    showTooltipsMid()
            ...
            MeasurementGraphType.Month.index -> {
                if (e.x == 4f) {
                    showTooltipsRight()
                } else {
                    showTooltipsMid()
            ...
            MeasurementGraphType.Year.index -> {
                if (e.x == 11f || e.x == 12f) {
                    showTooltipsRight()
                } else {
                    showTooltipsMid()
